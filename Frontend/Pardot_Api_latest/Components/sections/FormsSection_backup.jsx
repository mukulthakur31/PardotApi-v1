...
export default function FormsSection({ 
  ...
  if (formStats && formStats.length > 0 && !activeFormView) {
    // Filter forms based on date if filters are applied
    let filteredForms = formStats;
    
    if (filterType && (filterType !== "" || startDate || endDate)) {
      filteredForms = formStats.filter(form => {
        if (!form.last_activity) return filterType === "";
        
        const activityDate = new Date(form.last_activity);
        const now = new Date();
        ...
        if (filterType === "last_7_days") {
          ...
          return activityDate >= new Date(now.getTime() - 180 * 24 * 60 * 60 * 1000);
    ...
    const totalViews = filteredForms.reduce((sum, form) => sum + (form.views || 0), 0);
    const totalSubmissions = filteredForms.reduce((sum, form) => sum + (form.submissions || 0), 0);
    const avgConversionRate = filteredForms.length > 0 ? 
      (filteredForms.reduce((sum, form) => sum + (form.conversionRate || 0), 0) / filteredForms.length).toFixed(2) : 0;
    const topPerformer = filteredForms.reduce((best, form) => 
      (form.conversionRate || 0) > (best.conversionRate || 0) ? form : best, filteredForms[0]);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <p style={{ color: "#94a3b8", margin: 0 }}>Select a date range and click "Get Form Stats" to load your form analytics</p>
      </div>
    </>
  );
}